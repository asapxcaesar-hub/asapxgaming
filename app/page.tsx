...
import { byDate, features, news, reviews, upcomingGames } from '@/lib/content'

export default function HomePage() {
  const featured = byDate(news)[0]
  const latestNews = byDate(news).slice(1, 5)
  const latestReviews = byDate(reviews).slice(0, 3)
  const coming = upcomingGames().slice(0, 3)
  const opinions = byDate(features).slice(0, 3)
  ...
  return (
    <div className="grid gap-14">
        ...
        <ul className="grid gap-3 md:grid-cols-3">
          {coming.map((game) => (
            <li key={game.slug} className="border border-line bg-elevated p-4">
              ...
              <Link href={`/games/${game.slug}/`} className="mt-2 block font-semibold hover:text-accent">
                {game.title}
              </Link>
      ...
      <section>
        <div className="mb-4 flex items-end justify-between">
          <h2 className="font-display text-4xl">Featured / opinie</h2>
          <Link href="/features/" className="text-sm text-accent">
            Alle features
          </Link>
        </div>
        <div className="grid gap-6 lg:grid-cols-3">
          {opinions.map((item) => (
            <Link key={item.slug} href={`/features/${item.slug}/`} className="grid gap-3 border border-line p-4 hover:border-accent">
              <p className="text-xs uppercase tracking-[0.16em] text-accent">Achtergrond</p>
      ...
      <section className="border border-line bg-elevated p-6 md:p-8">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted">
          Streams en shorts staan op de kanalen van {site.creator.handle}. Geen nagemaakte
          thumbnails — de placeholders hieronder linken naar het echte profiel.
        </p>
        <div className="mt-5 grid gap-4 md:grid-cols-3">
          {videos.map((clip) => (
            <a key={clip.id} href={clip.href} target="_blank" rel="noreferrer" className="grid gap-2 hover:text-accent">
              <CoverPlaceholder label={clip.coverLabel} />
